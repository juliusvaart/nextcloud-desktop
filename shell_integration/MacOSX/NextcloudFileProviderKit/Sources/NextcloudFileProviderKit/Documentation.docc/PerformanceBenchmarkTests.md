...
How to reproduce the scan, ingestion and enumeration measurements the in-suite
benchmarks make, and what each one actually measures.

> Note: This article covers `PerformanceBenchmarkTests`, which runs against the
> package's mock remote interface. For measurements against a real server, and
> for comparing two revisions, see <doc:PerformanceBenchmarks>.
